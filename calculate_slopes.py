...
from constants import (AP_hist_folder, dt_new, hb_AP_interval, kn_AP_interval,
                       mins_per_frame, output_slopes_folder,
                       slope_length_frames, slope_length_mins)
...
def calculate_slopes(data):
    ...
    # nc 14 slope
    # get data points for the current dataset
    start_nc14 = 0
    end_nc14 = start_nc14 + slope_length_mins
    slope_nc14, coefs14 = get_slope(gene_data, 14, start_nc14, end_nc14)
    ...
    plot_slopes(gene_data, coefs14, start_nc14, end_nc14)
...
def get_slope(gene_data, nc, start_time, end_time):
    cur_data = gene_data[(gene_data.nc == nc)
                         & (gene_data.time >= start_time)
                         & (gene_data.time <= end_time)]
    if cur_data.count().nc <= 1:
        slope = np.nan
        coefs = [np.nan, np.nan]
    else:
        coefs = np.polyfit(cur_data.time, cur_data.intensity, 1)
        slope = coefs[0]
...
def plot_slopes(gene_data, coefs14, start_nc14, end_nc14):
    ...
    x_fit14 = np.asarray([start_nc14, end_nc14])
    y_fit14 = x_fit14 * coefs14[0] + coefs14[1]
    ...
    dt_new = 0.7
    avg_data = get_avg_on_regular_time_mesh(gene_data, dt=dt_new)
    # avg_data = gene_data.groupby(by='time').mean()
    # print(avg_data)
    ax.plot(avg_data.index, avg_data.intensity, '-o')
